...
**Post-run status (2026-07-04).** The two `[HYP]` residuals flagged above have since
been closed elsewhere: the dimensional magnitude bridge (channel radii, `ρ_iL`, gain
`g`) in `validation/synthetic/a3_dimensional_bridge.py` (FUTURE_WORK §A.3: metre-scale
radii derived, `g` bounded and direction-invariant), and the roughness-length
prefactor `z_0(λ,a)` in `validation/synthetic/a2_field_point.py` (FUTURE_WORK §A.2:
calibrated from Blumberg & Curl 1974's measured scallop-train friction, `B_L = 9.4` ⇒
`z_0 ≈ 0.023 L₃₂ ≈ 0.23 a` — 2–3× rougher per amplitude than the sand-grain analogy
assumed here).
