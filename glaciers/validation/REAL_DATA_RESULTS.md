...
### §V.1c variable-`φ` RTN from a MEASURED basal-water observable — the P4a §6.4 open step, executed

The papers' stated single open step for the variable-`φ` escape — "a `φ` field
from an **independent** observation of basal water (radar bed specularity)" —
is now run on real data: the **ICECAP HiCARS basal-interface specularity
content** profiles (Young et al. 2020, USAP-DC doi:10.15784/601371; 432
transects, 3.20 M points, East Antarctica ~80–170°E incl. Totten/Aurora/Byrd),
binned onto the 5 km-decimated Bedmap2 grid (14,126 covered grounded cells =
3.0% of grounded; 2,392 in the near-flotation band `H_af/H<0.7`; robust to
`min-pts` 3→10). Specularity is an established distributed-water/pressure proxy
(Schroeder et al. 2013, 2015; Dow et al. 2019). Constant-`φ` anchor reproduces
inside the coverage (`Spearman(RTN, H_af/H) = −1.0000`).

- **The observable is genuinely independent of flotation** — in-band
  `Spearman(spec-φ, H_af/H) = ±0.17` — so the reordering it induces carries
  information thickness-above-flotation does not have. Under the **pressure**
  reading (high specularity = distributed high-pressure water → high `φ`;
  Schroeder/Dow), the intrusion class changes on **162** covered cells (all
  newly flagged, median **10 km** from the grounding line; in-band
  `Spearman(RTN, H_af/H)` moves to −0.9956). Under the opposite
  **connectivity** reading (high spec → well-connected → low `φ`), **89** cells
  (all dropped, median 5 km). The two physically-arguable sign conventions move
  the near-GL classification in *opposite directions* — so the sharpened open
  step is no longer "find an independent observable" but "pin the spec→`p_w`
  sign with co-located borehole pressure or an intrusion survey".
- **The §H.1.1b routing escape is weakened**: on the same covered cells the
  Shreve-potential routed connectivity is **uncorrelated with measured
  specularity** (`Spearman ≈ ±0.04`, in-band +0.02). The model-`φ` reordering
  (in-band −0.983, 96 changed cells here) is therefore *not* validated as a
  basal-water proxy by radar; the measured-`φ` path is the defensible one.
- Caveats carried: regional coverage (flight lines, East Antarctica), 1 km
  L2 smoothing, 2008–2012 epoch vs Bedmap2, and specularity's known geometry
  sensitivity (Schroeder et al. 2015). No gridded intrusion survey exists, so
  this remains a provenance upgrade (measured vs model `φ`), not
  precision/recall.

Reproduce: `python validation/external/rtn_specularity_real.py --stride 5
--min-pts 3` (data: USAP-DC 601371 zip + Bedmap2 bin). Artifacts:
`validation/reports/rtn_specularity_real.{json,png}`.

### §V.1d the spec→`p_w` sign, pinned — three independent lines select the pressure reading

§V.1c ended at the sharpened open step: "pin the spec→`p_w` sign with co-located
borehole pressure or an intrusion survey". No such borehole exists in East
Antarctica, so `rtn_spec_sign_pin.py` pins the sign with **three independent
consistency lines whose predictions have opposite signs under the two
conventions** — one convention has to fail:

- **Line 1, the lake anchor (data; non-circular).** Active subglacial lakes are
  ponded water detected by *surface-height* anomalies (ICESat altimetry:
  Smith et al. 2009; Siegfried & Fricker 2018 outlines via USAP-DC 601470) — an
  observable fully independent of radar. By roof force balance ponded water
  carries the overburden (`p_w ≈ p_i`, `φ → 1`; Stubblefield et al. 2021 model
  lakes at `N ≈ 0`). **15 of the 131 catalogue lakes have ICECAP coverage (74
  cells)**: their median binned specularity is **0.108 vs 0.050** for the 13,410
  non-halo control cells — the median lake cell sits at the **74th percentile**
  of the control distribution, `P(spec_lake > spec_ctrl) = 0.69`,
  flotation-stratified label-permutation `p = 5×10⁻⁴` (iid MW `p = 5.6×10⁻⁹`),
  12/15 covered lakes above the control median, and the far-from-GL variant
  (>50 km, kills the GL-proximity confound) is unchanged (`p = 5×10⁻⁴`). A
  monotone *decreasing* spec→`p_w` map would assign its **lowest** pressures to
  exactly the cells physics pins at `p_w ≈ p_i` — untenable.
- **Line 2, the creep-persistence floor (physics; NR33).** Cells specular in
  BOTH ICECAP epochs (2008/09 and 2011/12; 259 cells sampled twice, **7
  persistent** at `spec ≥ 0.2` in both, 14/9/7/2 across thresholds
  0.10/0.15/0.20/0.30) are quiescent water bodies that survived ≈4 yr of Nye
  creep closure, which requires `N ≤ n(2At)^(−1/3) = 3.5 bar` — i.e.
  `φ ≥ 0.981–0.990` (median 0.9898; the sheet-geometry scaling sharpens this to
  ~0.1 bar / `φ ≥ 0.9998`). The floor binds only the *high-spec* cells, so it is
  an **orientation** constraint: an increasing map keeps a free low end
  (admissible range up to 1), while a decreasing map is `≥` its persistent-set
  value everywhere below the threshold — its whole range is squeezed above the
  floor (max range `1 − 0.981 = 0.019`). The connectivity convention's claimed
  range (0.17) exceeds its admissible range **9×** → inadmissible. (Both maps'
  absolute values sit below the floor at `PHI_HI = 0.97` — that is a
  top-of-scale calibration note, `PHI_HI ≥ 0.98` for persistent cells, not a
  sign discriminator.)
- **Line 3, lubrication consistency (data; confound-controlled, weakest).**
  Sliding laws have `u_b` increasing as `N` drops (Lliboutry 1968; Iken 1981;
  Schoof 2005). Sampling MEaSUREs phase-based velocity (NSIDC-0754, 450 m) on
  the 14,115 covered cells: the **raw** Spearman(spec, log v) is **−0.22**
  (specular cells concentrate in slow interior catchments), but at matched
  driving stress, thickness and flotation fraction (rank-residual partial) it
  **flips to +0.152 with a 50-km block-bootstrap CI [+0.106, +0.206]** (805
  blocks) — wetter-looking beds slide faster *at matched stress state*, the
  pressure reading's sign. The raw-vs-partial sign flip makes this line
  non-trivial (it is not repackaging an obvious raw correlation); it stays
  scoped as consistency evidence, not proof (sliding and hydrology are mutually
  causal).

**Verdict: `pressure_highspec_highphi` is the physically admissible monotone
reading** (all three lines; the connectivity reading fails all three). The
§V.1c pressure-convention numbers are therefore the primary ones: **162 covered
cells newly flagged at median 10 km from the grounding line**. Co-located
borehole pressure / a gridded intrusion survey remains the gold standard and
the papers still say so; what changes is that the sign is no longer *open* — it
is pinned by physics + two independent datasets, with the borehole as
confirmation, not arbiter.

Caveats carried: §V.1c coverage/epoch caveats; S&F outline epoch (2003–2016) vs
ICECAP epoch adds scatter that *dilutes* the lake anchor (conservative for the
one-sided test), as does 5-km binning of ~10-km lakes; the creep floor assumes
quiescence (melt-opening ≪ closure — R-channels evade it but are
low-specularity objects at km footprint); temperate `A` is the dominant
systematic (colder ice raises `N_max` only as `A^(−1/3)`).

Reproduce: `python validation/external/rtn_spec_sign_pin.py --vel-nc
<NSIDC-0754 nc>` (data: USAP-DC 601371 + 601470, Bedmap2 bin, NSIDC-0754 via
earthaccess). Artifacts: `validation/reports/rtn_spec_sign_pin.{json,png}`.
Unit-proofs: `glaciers/tests/test_rtn_spec_sign_pin.py` (10) +
`general_two_clocks/tests/test_new_relationships10.py` (8, NR33).

### §V.1e the specularity gauge in metres — Kirchhoff inversion, zero parameters

The observable itself is more than an index: reading specularity content as the
coherent power fraction of a Kirchhoff–Gaussian interface inverts in closed form,
`σ = (λ_ice/4π)·√(−ln s)` with `λ_ice = 2.81 m` (60 MHz in ice, `n = √3.17`) — an
RMS bed roughness **in metres per cell** at the ~65-m Fresnel scale, executing
horizon-ledger E1. Closed-form corollaries (all unit-proved): the gauge window
**5.1–38.7 cm** (ε = 0.05), the minimum-noise sweet spot at exactly `s* = e^{−1/2}`
(σ* = 15.8 cm), and Schroeder et al. 2015's independent scattering-model bound
"water interfaces ≲ 15 cm RMS" reproduced as the threshold `s ≥ 0.637`. On the
survey: point-level median **σ = 0.39 m** (censoring-aware; 21.8 % of points only
bounded at σ > 0.48 m), active-lake cells **0.333 m vs 0.387 m** for
flotation-matched controls (rank-exact transfer of the §V.1d anchor, stratified
`p = 5×10⁻⁴`), and the 61 water-like (σ ≤ 15 cm) cells sit deep inland (median
620 km from the GL) — the interior distributed-canal class, not altimetry lakes,
exactly where Young 2016 / Dow 2019 put it. Joint with NR33: the 7 persistent
cells read **σ ≈ 0.2–0.3 m AND φ ≥ 0.98** — the P4a §6.4 driver in physical
units. Dual-frequency falsifier: at MCoRDS 195 MHz the same beds must read
`s' = s^{10.6}`.

Reproduce: `python validation/external/e1_radar_roughness.py --bin-dir <bedmap2>
--spec-dir <601371> --lake-stats <601470 .dat>`. Artifacts:
`validation/reports/e1_radar_roughness.{json,png}`. Unit-proofs:
`glaciers/tests/test_e1_radar_roughness.py` (14, incl. first-principles MC of the
exp(−g) power convention and the censoring estimator). Full story:
`glaciers/REPORT_RADAR_ROUGHNESS.md`.
