...
def bushuk_adjustment_bound():
    ...
    *not* apply.
    ...
    return dict(
        ...
        superseded_by=("scallop_bushuk_raw.py / figures/78_bushuk_raw_pin.json "
                       "-- RESULT 25, the pin EXECUTED on the raw h(x,t) arrays "
                       "(M. Bushuk, pers. comm.): I(pre-registered)=3.63, "
                       "battery 0.51-6.50, flux-dynamic 1.20; downstream + "
                       "damped unanimous; flux parity break P(Im>=0)<1e-4. "
                       "Both this figure bound (I~0.10: equilibrium c and by-eye "
                       "tau) and the digitized 58b reading (I~10.3: wrong-train "
                       "27.9mm spacing, comb c 5x low) are superseded by the "
                       "raw-array measurement."),
    )
