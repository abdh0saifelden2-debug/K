...
# Part 9 — the two-clocks closure test in a subglacial cavity (real BEDMAP1 bed)
 
**Setup:** a 128² penalized pseudo-spectral DNS of turbulent meltwater in a cavity
between a **real Antarctic bedrock transect** (BEDMAP1 airborne radar, 220 km, 1757 m relief; warm, θ=1) and a flat ice base (cold, θ=0), spun up for
...
## 1. The flow (figure 35)
 
![fields](./figures/35_subglacial_fields.png)
...
The bed here is **not** synthetic: it is a measured BEDMAP1 airborne-radar bedrock transect (British Antarctic Survey / SCAR Bedmap, CC-BY-4.0; DOI:10.5285/f64815ec-4077-4432-9f55-0ce230f46029), mirrored for periodicity and non-dimensionalised to the cavity. Its real relief — asymmetric bumps and overdeepenings — sets where the flow separates.
 
## 2. Energy transfer and solenoidality (figure 36)
 
![transfer](./figures/36_subglacial_transfer.png)
 
| model | rel. RMS(∇·m) | transfer corr. with truth |
|---|---|---|
| truth | 9.10e-15 ± 4e-16 | 1.000 |
| Smagorinsky | 8.39e-15 ± 1e-16 | -0.707 ± 0.018 |
| surrogate | 6.72e+00 ± 2e-02 | 0.700 ± 0.029 |
| projected-FDT | 7.95e-15 ± 1e-16 | 0.453 ± 0.015 |
...
## 3. Where the wake is fed — spatial backscatter (figure 37)
 
![backscatter map](./figures/37_subglacial_backscatter_map.png)
...
*both* closures (Smagorinsky **-0.02 ± 0.00**,
projected-FDT **-0.07 ± 0.01**) — this is the
...
at the shell level (-0.71) and drains every scale, whereas
projected-FDT is positively correlated (+0.45) because it returns
energy in the shells where the truth backscatters. A purely dissipative K-theory
LES therefore drifts toward a "dead wake".
 
## 4. Lee heat trapping is counter-gradient (figure 38)
 
![heat flux](./figures/38_subglacial_heatflux.png)
...
a **heat-flux/heat-content proxy**, not a Stefan phase change; the bed geometry is now a **real measured transect** (BEDMAP1), but it is non-dimensionalised and made periodic to fit the spectral box; there is still no ice rheology and no coupled subglacial hydrology or sliding law. The
