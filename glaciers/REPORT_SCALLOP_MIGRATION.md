...
|---|---|---|
| resolved advective `−u·∇θ` | `−1.14×10⁻⁴` | migrates **downstream** |
| **uniform** eddy diffusivity `∇·(K∇θ)` | `−1.2×10⁻¹⁸` | machine zero (`~10⁻¹⁴` of resolved, `std=0`) |
| **flow-aware** Smagorinsky `K_eddy(|S|)` | `+2.0×10⁻⁶` | `~57×` below resolved, **wrong (upstream) sign** |
| **wall-scaled mixing length** `ℓ=κd`, `K=ℓ²|S|` | `+2.96×10⁻⁵` | `3.8×` below resolved, **wrong (upstream) sign**; outer-layer cap `ℓ≤0.2H` changes it `<0.1%` |

All battery members still **smooth** (`E_sin ≠ 0`): K-theory can damp but cannot
migrate **downstream** — the direction-blind member is exactly parity-protected,
and the flow-aware members inherit only the `|S|`-magnitude asymmetry, which
pushes their residual quadrature the wrong way (upstream, ≤26% of resolved;
`figures/59b_ktheory_closure_battery.json`, P3-R2). GPU/CuPy falsifiable controls (Tesla P100) confirmed (a) parity —
...
### 8.4 Digitized upgrade (2026-07-03): the figure bound's inputs were wrong — and the corrected index sits ABOVE the band

Bushuk didn't reply to the raw-array request (1 week), so the next-best step was
executed: **digitize the adjustment-regime panels directly** from the openly
hosted GFDL preprint (`gfdl.noaa.gov/wp-content/uploads/2019/05/Bushuk_JFM2019.pdf`,
sha256-pinned). `bushuk_fig3_digitize.py` locates the (image-only, halftone)
timeseries-figure page by content, calibrates each panel piecewise-linearly
through **visually verified tick-label anchors**, extracts the amplitude series
with a density-peak Viterbi tracker (robust to gridlines), and fits the
crest-position panel with a slanted-comb Rayleigh statistic (robust to the dashed
"anomalous advection" fragmentation). Committed artifacts:
`figures/58b_bushuk_digitized.{json,png}` + `tests/test_bushuk_digitized.py`.

**Digitized kinematics (exp 1b, raw adjustment row):**

| quantity | digitized | vs §8.1's figure-read |
|---|---|---|
| amplitude series | n=58 pts, 11.0 → 9.2 mm over ~300 min (early/late window quartiles), strongly oscillatory | — |
| `τ` (e-fold) | **2.6×10³ min** (damped-oscillator refit `A₀e^{−t/τ}[1+b·cos(2πt/T+φ)]`, R²=0.42, 68% CI 2.1–3.4×10³, T≈69 min, b≈0.03; preferred model) — battery: 1.2×10³ (envelope quartiles) … 2.9×10³ (log-linear, R²=0.25); two-exp control degenerates (R²=0.30) | `1–3 h` — **too fast by ≥5×** |
| `c_mig` | 0.17 mm/min (all-period comb); Bushuk's mean **0.11** (excludes anomalous periods) | 0.11 yes (kept) |
| `λ` (train spacing) | **27.9 mm** (6 crests, spacings 21–31.5 mm) | `13 cm` — **4.7× too large** (equilibrium-band value, not this train's) |
| `I = τ·c/λ` | **10.3** (preferred: oscillator τ × published c; estimator battery 4.9–17.4) | 0.10 (two large errors cancelling) |

**Honest verdict.**
1. **Sign structure still confirmed**: the train damps *and* migrates downstream
   (`c > 0`, net decay) — the frozen-probe form `s = −β + iω` in the right regime.
2. **Magnitude now overshoots**: every estimator combination sits **6–20× above**
   the solver band `[0.33, 0.88]` — the earlier "factor 2–3 below" bound rested on
   a τ read ≥5× too fast and a λ borrowed from the wrong train, which happened to
   cancel to a near-band value. §8.1's numbers are **superseded**.
3. **Two physical caveats on the overshoot**: (i) the adjusting train is far out
   of equilibrium with the post-cut flow (spacing formed at `U = 1.00 m/s`
   persists into `U = 0.16 m/s`, while the solver band describes near-equilibrium
   mode kinematics); (ii) the amplitude is strongly oscillatory — a
   single-exponential eigenmode is only weakly exhibited in-figure (R² = 0.25),
   so `τ` itself is estimator-dependent (the dominant spread in `I`).
4. **The pin is unchanged and still decisive**: `harmonic_mode_rate()` on the raw
   `h(x, t)` arrays (on request; co-author route: D. Holland / A. Stern) would
   replace both `τ` and `c` with the complex mode rate of the *same* mode and
   settle the magnitude question. Alternative datasets for the same test:
   Weady et al. 2022 (PRL 128, 044502) interface extractions; Cohen/Berhanu
   dissolution-scallop time-lapses (growth regime, sign caveat applies).

### 8.5 THE PIN, EXECUTED (2026-07-08): raw `h(x,t)` arrays received from Bushuk — RESULT 25

Bushuk replied and provided the raw interface arrays
(`melt_data_timestring_sub2.mat`, pers. comm.): 12 profiles (166 points,
148.6 mm window, 5-min cadence, `timestring` 19:17:02–20:12:02) **which are
exactly the fig-3(m–r) adjustment sequence** (experiment times 512–567 min;
12 × 5-min uniform cadence matches only that panel; the drive was cut
`U = 1.00 → 0.16 m s⁻¹` at 495 min). The file also carries Bushuk's own
melt/advection decomposition (`melt`, `x_advection`, `adv_grid`,
`z_profiles_advection`), used as independent cross-checks. Analysis:
`scallop_bushuk_raw.py`; artifacts `figures/78_bushuk_raw_pin.{json,png}`;
derived modal/flux statistics committed to
`subglacial/data/bushuk_raw_derived.json` (the raw .mat itself is not
redistributed); tests `tests/test_scallop_bushuk_raw.py` (17).

**The four measurements (one record, no digitization):**

| measurement | result |
|---|---|
| pre-registered pin (`harmonic_mode_rate`, verbatim) | `Re(s) = −2.8×10⁻⁵ s⁻¹` (τ = 604 min, **damped**), `dφ/dt = −6.3×10⁻⁴ rad s⁻¹` → `c_mig = +0.90 mm min⁻¹` (**downstream**), dominant mode = window scale (λ ≈ 149 mm), **`I = 3.63`** |
| flux-parity quadrature (per-interval `s_flux = −m_k/h_k`, 11 intervals) | `Im(s_flux) < 0` in **every** bootstrap resample: `P(Im ≥ 0) < 10⁻⁴` — **the parity break is in the real melt flux** (K-theory: `Im ≡ 0`); mean flux–topography phase `arg(m_k/h_k) ≈ 87°`; `I_flux = 1.20` (16–84%: 0.55–4.09) |
| migration-speed cross-checks | Bushuk crest tracking **0.953**, x-corr tracker **0.931**, modal phase **0.895**, flux quadrature **0.904 mm min⁻¹** — four readouts, **6.5% spread**, all downstream |
| controls | advected-frame residual phase speed **0.14%** of the lab-frame `c`; crest angle `φ = arctan(ḣ/c) = 14.8°` vs published **15°**; mean melt 0.253 mm min⁻¹ |

**Honest verdict.**

1. **The corrected-mode form is observed on real ice** — the train damps
   (`Re(s) < 0` verbatim + envelope + flux mean) **and** migrates downstream
   (unanimous across every estimator, window, taper and both harmonics). This
   was the K-theory-falsifying structure; it is now measured, not digitized.
2. **The parity break is measured in the flux itself.** The per-interval melt
   field carries a quadrature (`Im ≠ 0`, `p < 10⁻⁴`) whose implied phase speed
   (0.904 mm min⁻¹) **quantitatively accounts** for the kinematic migration
   (0.93 ± 0.03 mm min⁻¹) — the flux phase lag *is* the migration, closed to
   <10%. A local down-gradient closure returns `Im ≡ 0` (§5's machine-zero
   keystone): the closure-space null is rejected **on the laboratory record**.
3. **The magnitude lands in the pre-registered anchoring-mismatch bin, and the
   digitized §8.4 reading is superseded.** `I` battery: verbatim **3.63**
   (bin-2 6.50, Hann 4.24, halves 1.70/0.69, linear-detrend 0.51), dynamic
   `I_flux` 1.20 — i.e. `I_obs = O(0.5–6.5)`, central `1.2–3.6`. The digitized
   `I ≈ 10.3` rested on a 27.9 mm crest-comb spacing the raw arrays **rule
   out** (bin-5/~30 mm power is 0.2% of the ~149 mm window mode) and an
   all-period comb `c = 0.17` that under-read the true advection ~5×
   (0.95 measured three ways — the "anomalous advection" fragments were the
   real signal). The overshoot above the near-equilibrium solver band
   `[0.33, 0.88]` is the *expected* direction: the train's λ is anchored to
   the pre-cut flow's `u_*` (`Re_* ≈ 540` post-cut, far below the 2600–3400
   equilibrium band), and the solver's own `I(n_w)` trend rises toward
   shorter-than-equilibrium wavelengths. τ remains the honest dominant
   uncertainty (0.09–0.28 e-folds resolved in 55 min vs 0.33 cycles of phase —
   migration is far better resolved than decay in this window).
4. **What flips to [VERIFIED — real data]:** downstream sign, damping, the
   flux quadrature (`Im(s_flux) ≠ 0`), and the four-way `c_mig` agreement.
   **What stays regime-caveated:** the absolute `I` comparison to the
   near-equilibrium solver band (a regime-matched drive-cut solver run is the
   remaining like-for-like step).

### 8.6 The regime-matched solver comparison (P100 drive-cut batch): the smoothing-only solver does NOT reproduce the adjustment-regime `I` — and the mismatch isolates the fast re-phasing channel

`scallop_drive_cut.py` runs the like-for-like counterpart of exp 1b on the
moving-boundary solver: seed one mode, co-evolve at `U_hi = 3.0`, **cut the
drive to `0.16·U_hi`** (Bushuk's ratio), and fit the post-cut complex mode rate
exactly as RESULT 25 fits the data. Tesla P100 batch (CuPy), 16 runs
(`n_w ∈ {8,12}` × `a₀/λ ∈ {0.05, 0.10}` × 4 seeds), committed to
`subglacial/drive_cut_p100.json`:

| quantity | solver (quality-gated) | raw Bushuk arrays |
|---|---|---|
| pre-cut `I` (same protocol) | 0.39–1.87 (rises with `n_w`, consistent with the frozen band's `I(n_w)` trend) | — |
| **post-cut `I`** | **0.12–0.69** (best-gated rows 0.23–0.28); the cut LOWERS `I` 3–7× per seed | **1.2–3.6** |
| post-cut migration | slows (`E_cos ∝ U^{1/2}` loses ~60% at `0.16×` drive) | **accelerates ~8–9×** (0.11 → 0.95 mm min⁻¹; Bushuk's own crest angle 40°→15° with mean melt only ×2.5) |

**Honest verdict.** The smoothing-only solver moves the WRONG WAY through the
cut: its migration is carried entirely by the reattachment quadrature
(`∝ U^{1/2}`), so cutting the drive slows migration at roughly conserved
conduction damping, and `I_cut` lands 5–10× BELOW `I_obs`. In the real record
the cut *accelerated* the pattern speed ~9× above the equilibrium advection
velocity of the *stronger* pre-cut flow. The discrepancy isolates, quantitatively,
the channel this solver deliberately excludes (§8's sign caveat): the
**unstable/growth branch that re-phases the pattern toward the new equilibrium**
— Bushuk's "anomalous advection". Read in two-clocks terms: the equilibrium
advection speed (0.11 mm min⁻¹, differential melt of the locked state) is the
**slow, material clock**; the adjustment migration (0.95 mm min⁻¹) is a
**pattern re-phasing (fast) velocity** — a phase speed, not a material-transport
speed, which is why it can exceed the locked-state speed by an order of
magnitude while the drive is 6× weaker. Falsifiable follow-up: a solver
carrying the moving-boundary instability (Ashton–Kennedy branch) should
reproduce `c_adj/c_eq ≈ 8–9` at a `0.16×` drive cut; this one, by design,
cannot. The paper-3 claims are untouched (sign, parity quadrature, four-way
`c_mig` agreement are direction/structure claims, all confirmed on the raw
arrays); what changes is the *reading* of the `I` magnitude comparison — it is
cross-branch (adjustment/re-phasing vs decay-branch) and should not be scored
against the near-equilibrium band.
