...
class KTheoryProbeFlow(ProbeFlow):
    ...
    def _eddy_diffusivity(self, u, v):
        ...
        if self.eddy in ("mixing_length", "mixing_length_capped"):
            # wall-scaled mixing length: l = kappa_vk * d(x, y) (distance to the
            # nearest wall: local ice interface above, bed below), nu_t=l^2|S|
            # -- the standard boundary-layer K profile (wall-aware AND
            # flow-aware), but still a pure magnitude (no direction). The
            # capped variant applies the conventional outer-layer limit
            # l <= 0.2*H to rule out any mid-channel-l artefact.
            yy = xp.arange(cfg.ny) * sp.dy
            d_ice = xp.clip(self.y_ice_x[:, None] - yy[None, :], 0.0, None)
            d_bed = xp.clip(yy[None, :], 0.0, None)
            d = xp.minimum(d_ice, d_bed)
            l_mix = 0.41 * d
            if self.eddy == "mixing_length_capped":
                l_mix = xp.minimum(l_mix, 0.2 * float(cfg.y_ice_mean))
            ux, uy, vx, vy = sp.ddx(u), sp.ddy(u), sp.ddx(v), sp.ddy(v)
            s11, s22, s12 = ux, vy, 0.5 * (uy + vx)
            smag = xp.sqrt(2.0 * (s11 ** 2 + s22 ** 2 + 2.0 * s12 ** 2))
            return l_mix ** 2 * smag * self.fluid
        if self.eddy != "smagorinsky":
            raise ValueError(
                f"unknown eddy closure {self.eddy!r}; expected 'uniform', "
                "'smagorinsky', 'mixing_length' or 'mixing_length_capped'")
...
if __name__ == "__main__":
    import json
    r = ktheory_control(eddies=("uniform", "smagorinsky", "mixing_length",
