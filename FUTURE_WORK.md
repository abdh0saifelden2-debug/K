...
### G.2 Scallop-creep amplitude law — **monostable**, creep negligible [VERIFIED monostable + DERIVED structure; magnitudes DERIVED × LIT (RESULT 14); field pin EXECUTED on raw Bushuk arrays (RESULT 25)]
...
The important remaining gap is now **closed (RESULT 25, 2026-07-08)**: `I_obs` was
computed from a real scallop train — the raw Bushuk et al. (2019) exp-1b `h(x,t)`
arrays, provided by the author (`glaciers/scallop_bushuk_raw.py`,
`REPORT_SCALLOP_MIGRATION.md` §8.5). The train damps **and** migrates downstream
(unanimous; four migration readouts within 6.5%), the melt-flux quadrature is
nonzero at `P(Im≥0)<10⁻⁴` (the K-theory parity null rejected **on real ice**) and
accounts for the migration speed to <10%, and `I_obs = O(0.5–6.5)` (pre-registered
estimator 3.6; flux-dynamic 1.2) in the pre-registered anchoring-mismatch bin.
RESULT 14 has moved from *a discovery in the framework* to *a measured property of
a real ice–water interface*; the regime-matched (drive-cut) solver comparison is
the remaining like-for-like step. That distinction **is** the
...
### I.3 Critical-slowing-down early-warning signal for ungrounding **[DERIVED + demonstrated; field test EXECUTED — registered null with a measured sensitivity floor]**
...
**FIELD TEST EXECUTED (2026-07-09; quarterly ITS_LIVE v2, 2014–2025, 86 points).**
`temporal_csd_field.py` + `REPORT_TEMPORAL_CSD_FIELD.md`: **registered null** — the
near-GL ASE trunk points (n=31) show *negative* median rolling-variance and AC1 trends
(τ_var=−0.15, τ_ac1=−0.18; 0/31 pass the joint Fourier-surrogate criterion); the only
positive-trend group is the steady control (+0.47, 0/14 significant, and it carries the
largest ITS_LIVE sampling-density drift τ_npairs≈+0.5 — the flagged artifact direction).
Consistent with §I.6's *spatial* variance gradient on the same corridors: 12 yr of ASE
thinning moves `N` at a fixed point by far less than the 4→40 km along-flow `N` sweep,
so the honest product is a **sensitivity floor** — at quarterly ITS_LIVE precision no
ASE trunk point moved measurably toward the fold in 2014–2025; the decadal EWS fires
only closer to ungrounding or on longer records. Forecast retained; falsifier's
"approaching flotation" premise only weakly satisfied on this window.
...
### I.5 Tidal velocity admittance — a continuous third probe of `s_N(N)` **[DERIVED + VERIFIED on synthetic; field test EXECUTED — fundamental admittance face AS REGISTERED, 2f face not field-resolvable (mapped)]**
...
FIELD: `glaciers/validation/external/tidal_admittance_field.py`,
`glaciers/REPORT_TIDAL_ADMITTANCE_FIELD.md`, 10 unit proofs.)*
...
**FIELD TEST EXECUTED (2026-07-09; BAS Filchner-Ronne 29-station GPS + 750-d Rutford,
open licence).** `tidal_admittance_field.py`: joint gapped-segment harmonic LSQ
(methods cross-validated: Rutford MSf/Mf=4.8 vs Murray 2007's "about five"). Across 17
grounded stations / 6 streams the **fundamental fortnightly admittance ε_v(MSf) rises
toward the GL AS REGISTERED** (pooled exact Kendall τ=−0.49, p=0.007; within-stream
concordance 23/27, sign p=0.0002; Foundation strictly monotone 4.0→1.7 % over
3.6→47 km). The grounded response is **fortnightly-dominated** (MSf 40–230 mm vs M2
1–6 mm; MSf≫Mf) — the nonlinearity-generated intermodulation face. The literal 2f/1f
curvature ratio is **not field-resolvable**: M4 sits at the mm OTL/noise floor and
MSf/M2 measures differential *penetration* (M2 dies within ~10 km; MSf reaches
20–100 km), not local curvature. Honest limit: the observed ordering is consistent
with |s_N| steepening but not exclusive of upstream stress-transmission decay
(Rosier–Gudmundsson screening); the tides-only (m,R) inversion needs an independent
ΔN scale and stays deferred.

### I.6 SPATIAL early-warning — a single-snapshot ungrounding precursor **[DERIVED + VERIFIED on synthetic; field test EXECUTED — variance face SUPPORTED on the measurable segments, ξ face not detected]**

*(`glaciers/validation/synthetic/spatial_ews.py`, `glaciers/tests/test_spatial_ews.py`;
FIELD: `glaciers/validation/external/spatial_ews_field.py`,
`glaciers/REPORT_SPATIAL_EWS_FIELD.md`, `glaciers/tests/test_spatial_ews_field.py`.)*
...
**FIELD TEST EXECUTED (2026-07-09; real ITS_LIVE v2 + MEaSUREs NSIDC-0709).** Flowlines
seeded at the GL and integrated upstream on 4 streams (Thwaites/Pine Island/Smith +
grounded control Rutford; 376 points, 2 km sampling; the Whillans ice-plain target is
NOT testable — optical polar hole, 15 usable pairs total — deferred to InSAR/GPS §I.5).
Result: the **variance face is supported on the measurable segments** — pooling the
SNR≥10 trunk segments, the detrended noise-corrected relative fluctuation σ_rel RISES
toward the GL (weighted Kendall τ=−0.34, one-sided exhaustive circular-shift p=0.012;
survives quadratic detrend τ=−0.40/p=0.020) while the control shows the opposite sign —
but the **naive registered estimator is noise-confounded**: full-corridor σ_rel gradients
track the relative noise floor (noise-control τ≈+0.8, upstream slow ice), so binning raw
ITS_LIVE variance by distance would mislead without the noise column. The **ξ face is
not detected** (flat 10–18 km at |τ|≤0.16 despite a noise bias *favoring* the predicted
rise). Near-GL levels: σ_rel≈1.5–2.0 % (trunks; 30–41 m/yr) vs 3.1 % (control) — no
cross-stream fold-proximity ordering; the signature is within-stream. Partial
confirmation with the confound mapped: `REPORT_SPATIAL_EWS_FIELD.md`.
...
* **The one field point (CLOSED from published drag data, 2026-07-04).** Pinning `c_z`
  needs a scallop train with BOTH geometry `(λ,a)` AND an independent drag measure on
  the same train. That measurement has existed for fifty years: **Blumberg & Curl
  (1974, JFM 65:735–751, pp. 742–744)** measured the friction factor of stable
  fluid-selected scallop trains in a flume and expressed it as the rough-wall log-law
  constant `B_L = 9.4` with the Sauter-mean length `L₃₂` as the roughness scale — the
  same experiments that anchor the repo's `Re* ≈ 2200` wavelength law (consistent
  regime, no extrapolation). Exact conversion (`validation/synthetic/a2_field_point.py`,
  6 tests): `z_0 = L₃₂·e^{−κB_L} = 0.023·L₃₂` (κ=0.40), i.e. sand-grain equivalent
  `k_s ≈ 0.70·L₃₂`, and for depth-to-length `a/L ∈ [1/12, 1/8]`:
  `c_z = z_0/a ≈ 0.19–0.28` → **`α_s ≈ 5.6–8.4`, 2–3× ABOVE the assumed `[0.3, 3]`
  band** — scalloped walls drag substantially harder per unit amplitude than the
  sand-grain analogy suggested. The §A.3-anchor drag becomes `C_d = 3.1e-3` (vs the
  band's `[1.6e-3, 2.6e-3]`): a ≤20× prefactor correction compressed to ≤2.1× in
  `C_d` — the log-law buffering derived above, now used in anger. Caveats carried:
  `B_L` is Sauter-mean-referenced (Springer & Hall 2020) — matches the repo's
  stable-train `λ`; Goodchild & Ford 1971's differing natural-cave constants carried
  as the systematic (`B_L ± 1` ⇒ `z_0` ×/÷ 1.5, never back inside the old band);
  still not an in-situ subglacial measurement (none exists).

§A.2 moves from "[HYP, leans on LIT]" to **[two legs settled] + [the prefactor
CALIBRATED from Blumberg & Curl 1974 drag data: `z_0 ≈ 0.023 L₃₂ ≈ 0.23 a`,
tagged LIT-calibrated] + [in-situ subglacial confirmation the one remaining absence]**.
No GPU, no download.
