...
| 3 | [`paper3_scallop_parity_break.md`](paper3_scallop_parity_break.md) | Downstream scallop migration `Im(s)≠0` is a parity-symmetry break no eddy-diffusivity closure can produce (measured: `E_cos`→machine-zero under K-theory); constant-free field test `I=τ·c_mig/λ` **executed on the raw Bushuk arrays (RESULT 25)**: parity break on real ice at `P<10⁻⁴`, four-way `c_mig` agreement, `I_obs=O(0.5–6.5)` | morphodynamics, ice/cave/dissolution patterns, turbulence | **Strong + field-tested.** The one open gate is closed; remaining caveat is the regime-matched solver comparison. |
...
- **`pytest`: 718 passed, 2 skipped (2026-07-04 re-run with optional real datasets provisioned (Bedmap2, USAP-DC 601371/601470, NSIDC-0754); 656 on a bare checkout;
  includes the §I sliding-law cluster, the NR16–NR32 modules — through the drainage-response
  window NR32 — and the real-data §I lake test).
  The badges are now current (README: 718).
