...
pins `I = O(0.3–0.9)`. We then execute the field test on the raw laboratory
`h(x,t)` arrays of Bushuk et al. (2019, adjustment regime; provided by the authors):
the train damps and migrates downstream (unanimous across estimators; four
independent migration readouts agree to 6.5%), the per-interval melt flux carries a
nonzero quadrature (`P(Im ≥ 0) < 10⁻⁴` — the parity-symmetric closure null rejected
on real ice) that quantitatively accounts for the observed migration speed (<10%),
and the constant-free index lands at `I_obs = O(0.5–6.5)` (pre-registered estimator
3.6; flux-dynamic 1.2) in the pre-registered anchoring-mismatch bin of a
far-from-equilibrium train.
...
## 8. The gate, closed: the raw-array field test — and its regime-matched reading
...
**The pin, executed (RESULT 25).** Bushuk provided the raw arrays
(`melt_data_timestring_sub2.mat`, pers. comm.): 12 interface profiles at 5-min cadence
over a 148.6 mm window — exactly the published adjustment sequence (experiment times
512–567 min; drive cut at 495 min), including the authors' own melt/advection
decomposition as cross-checks. Running the **pre-registered** harness verbatim
(`harmonic_mode_rate`, self-validated to `<3%` on synthetic ground truth) plus a
robustness battery (`scallop_bushuk_raw.py`):

| measurement | result |
|---|---|
| dominant-mode complex rate | `Re(s) = −2.8×10⁻⁵ s⁻¹` (damped, τ ≈ 604 min), `c_mig = +0.90 mm min⁻¹` (downstream), λ ≈ 149 mm (window mode) |
| the constant-free index | **`I = 3.63`** (pre-registered estimator); battery 0.51–6.50 across detrend/taper/half-record/harmonic; flux-dynamic `I = 1.20` (16–84%: 0.55–4.09) |
| **flux-parity quadrature** | per-interval `s_flux = −m_k/h_k`: `Im(s_flux) < 0` in every bootstrap resample, `P(Im ≥ 0) < 10⁻⁴` — **the parity break, measured in the real melt flux** (a down-gradient closure gives `Im ≡ 0` exactly, §5) |
| dynamical closure | the flux quadrature's implied speed (0.904 mm min⁻¹) equals the kinematic migration (Bushuk tracking 0.953, x-corr 0.931, modal phase 0.895) to <10% — the phase lag *is* the migration |
| controls | advected-frame residual 0.14% of `c`; crest angle 14.8° vs published 15°; mean melt 0.253 mm min⁻¹ |

Three readings. (i) **The corrected-mode form is observed on real ice** — damping and
downstream migration together, the exact `s = −β + iω_mig` structure, now measured
rather than digitized. (ii) **The closure-distinguishing quadrature is nonzero in the
flux itself** at `p < 10⁻⁴` and accounts quantitatively for the migration — the
field-data analogue of the §5 operator swap, where the down-gradient closure sits at
machine zero. (iii) **The magnitude lands above the near-equilibrium solver band
`[0.33, 0.88]`, in the pre-registered anchoring-mismatch bin**: this train's λ is
anchored to the *pre-cut* flow (`Re_* ≈ 540` post-cut vs the 2600–3400 equilibrium
band), and the solver's own `I(n_w)` trend rises toward shorter-than-equilibrium
wavelengths; τ (0.09–0.28 e-folds resolved in 55 min) remains the dominant
uncertainty, whereas the migration phase (0.33 cycles) is well resolved. An earlier
figure-digitization estimate (`I ≈ 10.3`) is **superseded**: its 27.9 mm crest-comb
spacing is ruled out by the raw spectrum (power at ~30 mm is 0.2% of the ~149 mm
mode) and its comb speed under-read the advection ~5×.

**The regime-matched solver run (drive-cut batch, Tesla P100) makes the magnitude
reading precise.** Repeating the protocol on the moving-boundary solver — co-evolve
at `U_hi`, cut to `0.16·U_hi`, fit the post-cut mode identically — gives
`I_cut = 0.12–0.69` (16 runs; the cut *lowers* solver `I` 3–7× because its only
migration channel, the reattachment quadrature `∝U^{1/2}`, weakens with the drive),
whereas the real record's migration *accelerated* ~8–9× through the cut
(0.11 → 0.95 mm min⁻¹; Bushuk's own crest angle 40°→15°). The `I` comparison is
therefore **cross-branch**: the observed adjustment migration is dominated by
pattern **re-phasing toward the new equilibrium** (a phase velocity — Bushuk's
"anomalous advection"), the unstable/growth channel this smoothing-only solver
deliberately excludes. The sign, parity-quadrature, and speed-agreement claims are
branch-independent and stand; the adjustment-regime `I` magnitude should be scored
against a growth-branch solver (predicted `c_adj/c_eq ≈ 8–9` at a `0.16×` cut), not
the near-equilibrium band. (`glaciers/scallop_drive_cut.py`,
`glaciers/subglacial/drive_cut_p100.json`.)

---

## 9. Discussion and scope

This is a solver-verified, quantitative characterization of the closure — corrected
mode, operator-swap shortfall, temperature-free ratio — **now tested against a real
ice–water interface record** (§8): the mode form (damped + downstream), the flux
quadrature (`Im ≠ 0`, `p < 10⁻⁴`), and the four-way migration-speed agreement are
measured on Bushuk's raw laboratory arrays, and the regime-matched drive-cut solver
batch (P100) pins the one residual magnitude question as cross-branch (§8): the
adjustment migration is a re-phasing (fast-clock) velocity requiring the growth
branch this solver excludes. What remains open is narrower still: a growth-branch
solver test of `c_adj/c_eq ≈ 8–9`, the migration exponent's field value (needs a
multi-`U` field record), and laboratory ice is not a glacier — cave/ice-shelf
scallop trains remain the field frontier. The banked solver claims — `Im(s)→0` at
`U=0`, strictly sub-linear migration, exponent `→½` at high `Re_*` (central `0.52`)
— stand unchanged.
...
python glaciers/scallop_field_test.py # §6; figures/58_*.json (harness self-check + superseded figure bound)
python glaciers/scallop_moving_boundary_check.py # moving-boundary migration transfer + I(St-invariance)
python glaciers/scallop_bushuk_raw.py # §8 THE FIELD PIN on the raw arrays; figures/78_*.json
pytest glaciers/tests/test_scallop_amplitude_harmonics.py \
       ...
       glaciers/tests/test_scallop_bushuk_raw.py
```

## Data and code availability

All scallop-migration and harmonic-decomposition results regenerate from the scripts in the reproduce section; the field-test harness and its synthetic validation data are in the public repository at https://github.com/abdh0saifelden2-debug/K. The raw `h(x,t)` arrays of §8 were provided by M. Bushuk (GFDL, personal communication) and are not redistributed here; the committed derived modal/flux statistics (`glaciers/subglacial/data/bushuk_raw_derived.json`) suffice to re-verify every §8 verdict offline, and the analysis regenerates them bit-for-bit from the raw file when present.
