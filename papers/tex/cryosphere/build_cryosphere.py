...
.. warning:: **Source-of-truth hazard (2026-07).** The submission content now
   lives in the committed ``.tex`` files (NR-promotion paragraphs and later
   edits were made in tex only); the ``../*.md`` drafts are historical.
   Running this builder OVERWRITES the committed cryosphere ``.tex`` from the
   stale ``.md`` and silently destroys that content. Only run it if you have
   first back-ported all tex-only content into the ``.md`` source; otherwise
   edit the committed ``.tex`` directly and compile with pdflatex.
...
if __name__ == "__main__":
    print("WARNING: regenerating %s.tex from the historical .md source -- "
          "tex-only submission content (NR promotions and later edits) will be "
          "LOST unless it was back-ported to the .md first (see docstring)."
