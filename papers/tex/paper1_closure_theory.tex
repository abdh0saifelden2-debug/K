...
Eddy-diffusivity (``K-theory'') closures represent all unresolved turbulent transport with a single scalar diffusivity. It is established that such closures arise as the Markovian (short-memory) limit of a Mori–Zwanzig (MZ) reduction of the resolved dynamics (Chorin–Hald–Kupferman; Stinis; Parish \& Duraisamy 2017), and that the discarded backscatter can be reinstated with a divergence-free, dissipation-tied stochastic force (Leith 1990; Mason \& Thomson 1992; Schumann 1995). Building on these results, this paper contributes an operator-by-operator \textbf{audit} of exactly what a single diffusivity throws away, and two concrete, falsifiable measurements. The audit names four simultaneous simplifications hidden in $\nu _{t}$: (i) the MZ memory kernel collapsed to a delta in time, (ii) the spectral eddy viscosity frozen constant in wavenumber, (iii) the fluctuation–dissipation-linked orthogonal force deleted, and (iv) the sign restriction $\nu _{t}\ge 0$ that forbids backscatter — each mapped to a specific term of the generalized Langevin equation. Assembling the known repairs (a scale-selective spectral eddy viscosity, a Leray-projected FDT-tied stochastic force, and an exact Leray projection) into one closure form, we run an a-priori, frozen-field benchmark in a 256\ensuremath{^{2}} periodic box (sharp-spectral filter at $k_{c}=32$). The \textbf{structural} result is \emph{predicted, not fitted}: the projected force stays divergence-free to $RMS(\nabla \cdot m)\approx 1.7\times 10^{-14}$ and carries the correct sign of energy backscatter, whereas a spectrum-matched random surrogate reproduces the force spectrum yet breaks incompressibility ($RMS(\nabla \cdot m)\approx 12$) and randomises the transfer direction, and Smagorinsky is purely dissipative ($T(k)\le 0$ everywhere). The transfer-magnitude match to correlation \textbf{1.000} is a \emph{consistency check on a fitted quantity} — the backscatter amplitude $\Theta (k)$ is set to the measured net transfer — and is reported as such, not as a prediction. Two measurements are genuinely new. First, the closure's memory time is \textbf{scalar-independent}: $\tau _{c}(salt)/\tau _{c}(heat)\approx 1$ across a 100\ensuremath{\times} molecular-Lewis contrast even though the scalar Nusselt numbers track $Le$ strongly — the memory belongs to the \emph{flow}, as the MZ orthogonal dynamics require — and the implied clock-mismatch correction cuts a transient K-theory scalar-solver error \textasciitilde{}15\ensuremath{\times} while vanishing identically in steady turbulence. Second, when exact projection is replaced by hyperbolic (Dedner) divergence cleaning, the constraint residual collapses only inside a \textbf{finite design window} $2\lesssim \gamma \tau \lesssim 12$ and \emph{re-grows} beyond it: pushing the cleaning rate up to clean fast locally stalls the global low-wavenumber modes through the slow telegrapher root — an over-cleaning failure absent from the original cleaning analysis. The same a-priori benchmark, repeated in genuine \textbf{3-D} (128\ensuremath{^{3}} forced DNS, where vortex stretching is active), reproduces all three structural verdicts and exposes a failure 2-D cannot see: the true subgrid flux is up-gradient (backscatter) over \textbf{\textasciitilde{}48 \%} of space, which a positive-definite eddy viscosity represents at \textbf{0 \%} by construction. Scope is explicit: a-priori (frozen-field), periodic, in 2-D and 3-D; the clean FDT/projection commutation is special to the spectral box and does not survive solid walls --- a wall-bounded a-priori test confirms exactly this: the filter--projection commutator $[F,\mathbb{P}]$ (machine-zero in the box) jumps to $O(10^{-2})$ in a channel and concentrates in the near-wall layer ($34\times$ the bulk), a global-pressure correction structurally invisible to a local eddy viscosity. A time-integrated \textbf{a-posteriori} test of a predictive version of the closure (§5b) confirms every variant is stable, that Smagorinsky is markedly over-dissipative, and that the FDT backscatter helps in the predicted direction — while honestly bounding the benefit: in pure 2-D no eddy-viscosity closure beats no model on the resolved spectrum (the resolved scales need near-zero net subgrid dissipation), and in a 3-D 7-seed ensemble at mild subgrid load (§5e) every variant is likewise stable and Smagorinsky over-dissipative, while the resolved-spectrum margin over no-model is at realization noise ($p=0.69$) --- the a-priori forward-cascade direction ($\langle \Pi \rangle >0$, backscatter over $\sim$48\% of space, §5c) being the robust 3-D structural result. The contribution is the audit, the scalar-independent memory time, the cleaning window, and the a-posteriori stability/over-dissipation bound — not a universal closure.
...
\item An \textbf{a-posteriori, time-integrated test} of a \emph{predictive} version of the closure (§5b): every variant is stable over \textasciitilde{}24 eddy-turnovers, Smagorinsky is confirmed markedly over-dissipative, the FDT backscatter helps in the predicted direction, and the honest 2-D ceiling — no eddy-viscosity closure beats no-model on the resolved spectrum — is established and explained; in 3-D (§5e) every variant is likewise stable and Smagorinsky over-dissipative across a 7-seed ensemble, while the resolved-spectrum margin over no-model sits at realization noise.
...
\caption{Wall-bounded a-priori test (Part 9d). The filter--projector commutator $[F,\mathbb{P}]$ is machine-zero in the periodic box but $O(10^{-2})$ in the channel, peaking \emph{at} the wall ($34\times$ the bulk): a global-pressure (elliptic-clock) correction confined to the near-wall layer, structurally invisible to a local eddy viscosity.}
\end{figure}
...
\textbf{Scope.} An a-priori operator-structure test on a synthetic multi-scale channel field: it isolates the filter/projector commutator --- a property of the operators and the wall geometry, not of any particular turbulence state --- and is \textbf{not} an a-posteriori channel-DNS run. It converts the ``does not survive solid walls'' caveat of \S5 into a \emph{measured, located} structural term: the wall correction is real, near-wall, and K-theory-invisible. (\code{general\_\allowbreak{}two\_\allowbreak{}clocks/\allowbreak{}run\_\allowbreak{}closure3d\_\allowbreak{}bounded.\allowbreak{}py}.)


\medskip\hrule\medskip


\section*{5e. The a-posteriori test in 3-D --- stability confirmed, the resolved-spectrum margin at seed noise}
...
\textbf{Every closure is stable in 3-D and Smagorinsky is again over-dissipative} (resolved kinetic energy drained to $0.87$--$0.98$ of the filtered DNS across seeds), reproducing the two robust a-posteriori verdicts of §5b. \textbf{The resolved-spectrum margin over no-model, however, does not survive the ensemble.} The no-model L2 error itself scatters roughly $4\times$ across seeds ($0.047$--$0.177$), and the paired log-ratio of the Smagorinsky to the no-model resolved-spectrum error is $+0.022\pm 0.052$ (SE), $t(6)=0.42$, $p=0.69$ --- Smagorinsky beats no-model in only \textbf{3 of 7} seeds. The best closure per seed alternates between Smagorinsky and the scale-selective/cusp eddy viscosities, and its winning margin is typically $\le 0.05\%$, i.e.\ within realization noise. A single seed (seed 0) showed Smagorinsky as the clear best (a $5.9\%$ margin); the ensemble shows that margin is a fluctuation, \emph{not} a $2\text{-D}\!\to\!3\text{-D}$ sign reversal. The a-priori §5c forward-cascade direction ($\langle \Pi \rangle >0$) still says a positive eddy viscosity should help where the subgrid load is large enough; the variational reading $\nu _{t}^{\mathrm{opt}}=\langle \Pi \rangle /(2\langle |S|^{2}\rangle )$ is $\approx 0$ in 2-D and \emph{positive} in 3-D, but at this mild load the a-posteriori resolved-spectrum benefit is not resolved out of noise.


\textbf{Honest scope.} What is robust in 3-D is the \emph{stability} of every variant and Smagorinsky's over-dissipation (7/7 seeds); what is \emph{not} resolved out of realization noise at this configuration is whether any eddy viscosity beats no-model on the resolved spectrum. The forcing is low-$k$ and $k_{c}=24$ captures most of the energy, so the subgrid load is mild and the margins are small. A controlled forcing-band sweep at fixed $k_{c}=24$ ($k_{f}=2.5/10/17$, all else identical) shows the margin tracks that \emph{subgrid load}: it is largest under scale separation (low-$k$ forcing) and \emph{vanishes} as the forcing nears the cutoff, where the resolved forced scales --- not the subgrid --- carry the energy and a subgrid model only over-damps them (at $k_{f}=17$, $\langle$KE$\rangle$ falls $18\times$ and no closure beats no-model). Settling the a-posteriori \emph{sign} therefore requires ensemble-averaged spectra at higher subgrid load (lower $\nu$, higher resolution), not this mild-load config; the a-priori direction result of §5c ($\langle \Pi \rangle >0$, backscatter over $\sim $48\% of space) is unaffected and stands as the paper's 3-D structural claim. At this mild load the FDT backscatter adds no further a-posteriori spectral gain ($\mathrm{specEV{+}bs}\approx \mathrm{specEV}$) --- consistent with it being the a-priori/structural repair of §5/§5c (the transfer \emph{sign} and solenoidality at one instant), not a resolved-spectrum a-posteriori effect. This \textbf{closes the §8.1 a-posteriori item}: 2-D bounded (no win) and 3-D stable with the resolved-spectrum margin at seed noise. (\code{general\_\allowbreak{}two\_\allowbreak{}clocks/\allowbreak{}closure\_\allowbreak{}aposteriori3d.\allowbreak{}py}.)
...
\textbf{6.1 $\tau _{c}$ measured, with sign.} From the autocorrelation of the SGS eddy-diffusivity in a developed run, $\tau _{c}\approx 0.02–0.03$ (in eddy-turnover units) with a \textbf{definite positive sign} (Mori 1965; Zwanzig 1973).
...
\begin{enumerate}
\item §5c now runs the a-priori benchmark in \textbf{3-D} (forward cascade, vortex stretching active): the structural verdicts survive and the \textasciitilde{}48 \%-of-space backscatter that Smagorinsky misses is exposed. §5b runs the a-posteriori test in 2-D (stable; Smagorinsky over-dissipates; the structured closure matches but does not beat no-model, because 2-D resolved scales need near-zero net SGS dissipation); \textbf{§5e now runs it in 3-D across a 7-seed ensemble} --- every variant stable and Smagorinsky over-dissipative in all seeds, while the resolved-spectrum margin over no-model is at realization noise ($p=0.69$); the a-priori $\langle\Pi\rangle>0$ of §5c remains the robust 3-D result. The remaining open item is the \textbf{wall-bounded a-posteriori} (time-integrated) test (§5d is a-priori only); if long-time tracking drifts there while the structural diagnostics stay correct, restore the finite-memory kernel (the t-model), which §3–§4 already name.
...
Schumann, U. (1995). Stochastic backscatter of turbulence energy and scalar variance by random subgrid-scale fluxes. \emph{Proceedings of the Royal Society A} 451, 293–318. doi:10.1098/rspa.1995.0126
