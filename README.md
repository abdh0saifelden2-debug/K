...
[![Tests](https://img.shields.io/badge/tests-1264%20passing-brightgreen)](#tests)
...
| 10g | **Result** | **RESULT 14** — wall-flux harmonic decomposition of the scallop amplitude law: conduction `β/a` is `K`-independent (`K²` curvature ansatz **falsified**), no `+α a^{1/2}` growth (smoothing-limited), and the flow channel is a **quadrature migration** term `∝U^{0.5–0.8}`. Corrected mode `s=−β+iω_mig`; migration is a **parity-symmetry break no K-theory closure can produce** (keystone control measured on P100). Constant-free, `ΔT`-free field test `I=τ·c_mig/λ`. **RESULT 22** generalises the swept window: across `a₀/λ∈[0.05,0.40]` the smoothing-only (no `+α`) and falsified-`K²` verdicts hold at **every** amplitude (`amplitude_generalization_scan`); **RESULT 23** closes the other half of Caveat D — out to drive `U=6` (beyond the swept `U∈[1.5,3.0]`) the smoothing-only verdict survives and migration keeps its **sub-kinematic** `∝U^{0.5–0.8}` scaling (`U^{+0.48}`, never `U¹`; saturates at `U≳4.5`) (`drive_window_scan`). **RESULT 25 executes the field test on the raw Bushuk-2019 adjustment arrays** (provided by the author): damped + downstream unanimous, melt-flux parity quadrature `Im≠0` at `P<10⁻⁴` accounting for the migration speed to <10%, `I_obs=O(0.5–6.5)` (pre-registered 3.6) in the pre-registered anchoring-mismatch bin — the parity break measured on real ice (`glaciers/scallop_bushuk_raw.py`) | 2D penalized DNS (CPU + P100) + **real laboratory `h(x,t)`** | [`glaciers/REPORT_SCALLOP_MIGRATION.md`](glaciers/REPORT_SCALLOP_MIGRATION.md) (`glaciers/scallop_amplitude_harmonics.py`, `glaciers/scallop_ktheory_control.py`, `glaciers/scallop_bushuk_raw.py`) | **Fig 56, 78** |
...
│   └── tests/
│
├── working_notes/                 ← provenance / reproduction / submission-stage notes
│   └── README.md                  │  what each note records
│
├── requirements.txt
...
1269 tests (1264 passed, 5 skipped on a fresh toolchain) covering signal processing, compressible solver accuracy, stability
...
loader, Holland fit, real-vortex margin), plus the derived cross-relationships
(NR1–NR42, including NR37's pseudo-sound M²→M¹ density crossover, NR38's
Randles-impedance bed, NR39's imaginary-coherence projection lemma, NR40's
Omori diffusion-kernel tail, NR41's cross-domain diffusive-visibility
window, and NR42's grid-frequency drainage window — NR32's band-limited
two-clocks transmission confirmed structural on the power-grid swing equation, and
NR43's independent ICESat-2/ATL15 confirmation that active subglacial lakes are the
episodic fast-clock on the secular slow-clock of ice dynamics; the line now
extends through NR71, including the response-anatomy sequence NR47–NR59, the
spin/EPR/polarization-entropy sequence NR60–NR67 on real ANDRO/EEG/solar data,
the tidal-EIS ladder NR64–NR65 on real BAS GPS, and the atmospheric two-clocks
sequence NR68–NR72: the turbopause as the flow-clock/molecular-clock handoff on
NRLMSIS 2.1, the exobase as the Knudsen memory-time crossover where local closure
dies, the escape ladder's series/parallel clock algebra reproducing
diffusion-limited H escape and the classical helium problem, the ambipolar
field as the quasineutrality constraint's divergence-cleaning multiplier on
IRI2016, and the F2-peak audit R(hmF2) that separates balance-formed from
dynamics-formed ionospheric layers by three decades — see [`papers/CROSS_RELATIONSHIPS_INDEX.md`](papers/CROSS_RELATIONSHIPS_INDEX.md)). No
downloaded data required — tests use synthetic signals, the bundled Otis best track,
committed model/data caches, and short in-process spinups.
...
| 58 | `58_scallop_field_test.json` | RESULT 14 constant-free field test: `I=τ·c_mig/λ` (harmonic-pin self-check + superseded Bushuk figure bound) | 10g/§G.2 |
| 78 | `78_bushuk_raw_pin.{json,png}` | **RESULT 25**: the field pin executed on the raw Bushuk exp-1b `h(x,t)` arrays — damped + downstream, flux parity break `P(Im≥0)<10⁻⁴`, `I_obs` battery | 10g/§G.2 |
