...
**External benchmark (2026-07-04, `nr29_jhu_benchmark.py`).** The zero-parameter law
was re-tested on the community-standard JHTDB `isotropic1024coarse` DNS (Re_λ≈433;
Li et al. 2008), fetched as full-domain periodic 64³ lattices (stride-16; aliasing
disclosed in the module docstring) at two decorrelated timepoints through the public
demo service. Result: `P(Π<0) = Φ(−⟨Π⟩/σ)` tracks the measured backscatter fraction
to **max |err| = 0.0076 over six (t, k_c) combinations** (k_c = 8/12/16 × t = 0.0/4.0),
with ⟨Π⟩ > 0 (forward cascade — the NR31 3-D premise) in all six. The law is not a
house-solver artifact. Artifacts: `figures/nr29_jhu_benchmark.{json,png}`; offline
unit-proofs `tests/test_nr29_jhu_benchmark.py` (6: sharp-filter exactness, Π≡0 at
identity filter, Galilean invariance, Gaussian exactness, K-theory limit, synthetic
tracking).
