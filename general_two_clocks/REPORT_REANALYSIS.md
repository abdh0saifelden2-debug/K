...
## Second-reanalysis cross-check (P0-R7; `run_reanalysis_r2.py`)

Same window (2021, t0=100, 24 daily means), same spherical-harmonic Helmholtz
split, independent assimilation system (NCEP-DOE Reanalysis 2, Kanamitsu et al.
2002): `KE_div/KE_rot` = 4.47% / 0.77% / 1.13% at 850/500/250 hPa vs R1's
3.99% / 0.67% / 1.04% — same ordering, same 500-hPa minimum, <=15% relative
differences (`figures/27b_reanalysis_crosscheck.json`). The ratio is a property
of the analyzed atmosphere, not of one reanalysis.
