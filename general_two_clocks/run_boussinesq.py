...
def fig_spde_limit(solver, st, out_dir):
    ...
    # P0-R6: seed ensemble -- one surrogate is an anecdote; 100 give the
    # distribution of the structural-ceiling numbers (correlation with the
    # true correction; residual divergence).
    n_ens = 100
    corrs, rmss = [], []
    ...
    ens = dict(
        n_surrogates=n_ens,
        seed_scheme="phase_randomized(ud/vd, seed=1000+2s / 1001+2s)",
        corr_median=float(np.median(corrs)),
        corr_abs_median=float(np.median(np.abs(corrs))),
        corr_abs_p95=float(np.percentile(np.abs(corrs), 95)),
        corr_ci95=(float(np.percentile(corrs, 2.5)),
                   float(np.percentile(corrs, 97.5))),
        rms_surr_median=float(np.median(rmss)),
        rms_surr_ci95=(float(np.percentile(rmss, 2.5)),
                       float(np.percentile(rmss, 97.5))),
        rms_proj=float(rms_proj),
        single_realization=dict(corr=corr, rms_surr=float(rms_surr),
                                seeds=(11, 12)),
    )
    import json as _json
    with open(out_dir / "24_spde_limit_ensemble.json", "w") as fh:
        _json.dump(ens, fh, indent=2)
    ...
    axes[0].set_title(f"spectra identical (Parseval)\ncorr(fields) = {corr:.3f} ~ 0 "
                      f"(|corr| p95 = {ens['corr_abs_p95']:.3f}, n={n_ens})")
    ...
    fig.savefig(path, dpi=130, bbox_inches="tight")
    plt.close(fig)
    return path, corr, rms_proj, rms_surr, ens
...
def write_report(report_path, fig_paths, stats, ens=None):
    ...
    if ens is not None:
        lines.append(
            f"- **Ensemble (not an anecdote):** over {ens['n_surrogates']} "
            f"independent phase-randomized surrogates, |corr| has median "
            f"{ens['corr_abs_median']:.4f} and 95th percentile "
            f"{ens['corr_abs_p95']:.4f}; residual RMS div is "
            f"{ens['rms_surr_median']:.2e} "
            f"[95% {ens['rms_surr_ci95'][0]:.2e}, {ens['rms_surr_ci95'][1]:.2e}] "
            f"(`figures/24_spde_limit_ensemble.json`).\n")
    lines += [
...
def main():
    ...
    p24, corr, rms_proj2, rms_surr, ens = fig_spde_limit(solver, st, out_dir)
    for p in (p22, p23, p24):
        print(f"  -> {p}")
    print(f"  div(u*)={rms_star:.2e}  div(P u*)={rms_proj:.1e}  "
          f"corr(surrogate)={corr:.3f}  div(surrogate)={rms_surr:.2e}")
    print(f"  ensemble (n={ens['n_surrogates']}): |corr| median "
          f"{ens['corr_abs_median']:.4f}, p95 {ens['corr_abs_p95']:.4f}; "
          f"div RMS {ens['rms_surr_median']:.2e} "
          f"[{ens['rms_surr_ci95'][0]:.2e}, {ens['rms_surr_ci95'][1]:.2e}]")

    write_report(Path(args.report), [p22, p23, p24],
                 (rms_star, rms_proj, corr, rms_surr), ens)
